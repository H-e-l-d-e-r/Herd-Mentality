...
public class RadioHijackModule : MonoBehaviour
{
    ...
    public void ExecuteHijack()
    {
        ...
        {
            Debug.Log("[HIJACK] Echec : Aucune emission detectee sur cette frequence. Pourquoi tu pirate du vide ?!");
            return;
        }
        ...
        {
            // SUCCES
            RadioManager.Instance.AddAudimatBonus(AudimatReward);
            Debug.Log($"<color=green>[HIJACK] REUSSITE ! Le signal est a nous. +{AudimatReward} Auditeurs !</color>");
        }
        else
        {
            // ECHEC
            Debug.Log("<color=red>[HIJACK] ECHEC ! Ils nous ont découvert et nous ont devancé.</color>");
        }
